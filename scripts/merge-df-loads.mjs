...
// --- Write HTML diff report to public/ for admin dashboard download ---
const mdToHtmlTable = (mdLines) => {
...
for (const line of lines) {
  if (line.startsWith('## ') && currentSection.length) {
    htmlSections.push(currentSection);
    currentSection = [line];
  } else {
    currentSection.push(line);
  }
}
if (currentSection.length) htmlSections.push(currentSection);

const htmlBody = htmlSections.map(section => {
  const heading = section[0]?.replace(/^#+\s*/, '') || '';
  const tableHtml = mdToHtmlTable(section);
  const textLines = section.slice(1).filter(l => l && !l.startsWith('|') && !l.match(/^\|[-| ]+\|$/));
  return `<section>
    <h2>${heading}</h2>
...
<head>
  ...
  <title>DirectFreight Load Diff — ${date}</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 960px; margin: 0 auto; padding: 40px 24px; color: #1a1a1a; background: #f9fafb; }
    h1 { font-size: 24px; font-weight: 700; margin-bottom: 4px; }
    h2 { font-size: 14px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #6b7280; margin: 32px 0 12px; }
    p { color: #374151; font-size: 14px; margin: 2px 0; }
    section { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 20px 24px; margin-bottom: 20px; }
    table { width: 100%; border-collapse: collapse; font-size: 13px; margin-top: 8px; }
    th { text-align: left; padding: 8px 12px; background: #f3f4f6; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; border-bottom: 1px solid #e5e7eb; }
...
<body>
  <h1>DirectFreight Load Diff</h1>
  <p class="meta">Run date: ${date} &nbsp;·&nbsp; Generated by Haul Monitor</p>
  ${htmlBody}
</body>
</html>`;

const htmlReportPath = resolve(__dirname, '../public/df-loads-diff-latest.html');
writeFileSync(htmlReportPath, htmlReport);
console.log(`🌐 HTML report → public/df-loads-diff-latest.html`);
