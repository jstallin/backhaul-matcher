...
// One shared component for v1 and v2; the host passes a palette so it matches
// either theme. Renders the share icon button + chooser + channel dialogs.
...
export function LoadShareMenu({ match, request, fleetHome, palette }) {
  ...
  const handleCopy = async () => {
    setOpen(false);
    try {
      await navigator.clipboard.writeText(buildCopyText({ match, request }));
      setCopied(true);
      clearTimeout(copiedTimer.current);
      copiedTimer.current = setTimeout(() => setCopied(false), 2500);
      // Log the share; a logging failure shouldn't disturb the copy UX.
      shareLoad({ channel: 'copy', match, request, fleetHome, user }).catch((err) =>
  ...
  const noteMax = mode === 'email' ? NOTE_MAX_EMAIL : NOTE_MAX_TEXT;

  return (
    <div ref={wrapRef} style={{ position: 'relative', display: 'inline-flex' }}>
      <button
        onClick={() => setOpen((o) => !o)}
        title="Share this load"
        aria-label="Share this load"
      ...
      {copied && (
        <span style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, padding: '6px 12px', background: '#16a34a', color: '#fff', borderRadius: '8px', fontSize: '12px', fontWeight: 700, whiteSpace: 'nowrap', zIndex: 30000, display: 'flex', alignItems: 'center', gap: '5px' }}>
          <CheckCircle size={13} /> Load copied
        </span>
      )}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: p.cardBg, borderRadius: '14px', width: '100%', maxWidth: '420px', padding: '22px', border: `1px solid ${p.border}`, boxShadow: '0 16px 48px rgba(0,0,0,0.35)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ fontSize: '16px', fontWeight: 800, color: p.text, display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {mode === 'email' ? 'Email this load' : 'Text this load'}
              </div>
              ...
              <>
                ...
                {mode === 'email' && (
                  <div style={{ marginTop: '8px', fontSize: '12px', color: p.textMuted }}>
                    Includes full load details and a route map. Replies go to {user?.email}.
                  </div>
                )}
